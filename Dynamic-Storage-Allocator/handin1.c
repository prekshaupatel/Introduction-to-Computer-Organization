...
void *mm_malloc(size_t size) 
{
  ...
  if (size <= DSIZE)
    asize = DSIZE*2 + OVERHEAD; /*we make the minimum size 2*DSIZE+OVERHEAD to make it convinient for our implementation*/
  else
...
void mm_checkheap(int verbose) 
{
  ...
  if (verbose)
    ...
    checklinkedlist(); //this check the linked list and if each node is valid
...
static void *placeend(void *bp, size_t asize)
{
  ...
  if (diff >= 24) {    /*if we have enough excess space to allocate a free block we leave the block in the free list*/
    PUT(bp-WSIZE, PACK(diff, 0));
    void * p = bp+diff;
  ...
  else {  /* otherwise we remove the block from the freelist and give all the space to malloc*/
      remove_node(bp);
    PUT(bp-WSIZE, PACK(csize, 1));
    PUT(bp+csize-DSIZE, PACK(csize, 1));
    return bp;
...
/*remove the node from the free list and connect the previous and next node*/
static void remove_node (void * p)
{
  void * prevb = *(void**) p;
...
/*add a node to the beginning of the freelist*/
static void add_node (void * p)
{
  if (freelist == NULL) {
    freelist = p;
